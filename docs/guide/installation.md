...
4. If `MCP direct`, do you already have a Codebase MCP API key?
   - If not, guide user to create one in `https://codebase.blog/settings/api-keys`.

### Step 1) Explain SKILLS vs MCP direct (brief)

Tell the user:

- Functionality is mostly the same in practice (both use the same MCP backend capabilities).
- Main difference is setup complexity:
  - `SKILLS`: easier setup and fast start.
  - `MCP direct`: more manual control over token/config/security policy.
- Output tone/format can differ slightly by AI agent behavior.
...
Use `<YOUR_API_KEY>` and configure only selected agents.
